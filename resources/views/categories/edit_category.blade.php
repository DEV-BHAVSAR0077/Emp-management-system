        ...
        <div class="panel-body">
            ...
            <form method="POST" action="{{ route('categories.update', $category) }}" novalidate>
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="name" name="name" value="{{ old('name', $category->name) }}" placeholder="Main Category Name" class="{{ $errors->has('name') ? 'input-error' : '' }}" required />
                ...
                <div class="sub-categories-section form-group">
                    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 10px;">
                        ...
                        <button type="button" class="btn btn-ghost btn-sm btn-add-sub" data-target="edit-category-subs">
                    ...
                    <div id="edit-category-subs" style="display: flex; flex-direction: column; gap: 10px;">
                        @php
                            $subs = old('sub_categories') ? old('sub_categories') : $category->subCategories->toArray();
                        @endphp

                        @foreach($subs as $index => $sub)
                            <div class="sub-category-row form-row" style="margin-bottom: 0; display: flex; gap: 10px;" data-id="{{ $sub['id'] ?? '' }}">
                                @if(!empty($sub['id']))
                                ...
                                <button type="button" class="btn btn-danger btn-remove-sub" data-db-id="{{ $sub['id'] ?? '' }}" style="flex-shrink: 0; display: flex; align-items: center; justify-content: center; padding: 0 15px;">
